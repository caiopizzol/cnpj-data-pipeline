...
Um ZIP sem arquivos reconhecidos ou um CSV reconhecido sem linhas interrompe a execução, sem marcar esse arquivo como processado nem publicar um novo manifesto. Isso não desfaz lotes já gravados no PostgreSQL. Use `KEEP_DOWNLOADED_FILES=true` para guardar os arquivos temporários ao investigar uma falha. ZIPs guardados incluem o mês no nome, como `2024-03.Cnaes.zip`. Caches antigos sem mês são ignorados; podem ser removidos manualmente.
