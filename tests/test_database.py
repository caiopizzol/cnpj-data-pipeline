...
class DatabaseProbe(Database):
    ...
    @property
    def truncated_tables(self) -> set[str]:
        return self._truncated_tables


@pytest.fixture
def db():
    """Create a Database instance with a test URL."""
    return DatabaseProbe("postgresql://user:pass@localhost:5432/testdb")


@pytest.fixture
def connected_db(db: DatabaseProbe):
...
class TestConnect:
    ...
    def test_connects_successfully(self, mock_connect: MagicMock, db: DatabaseProbe) -> None:
        mock_conn = MagicMock()
    ...
    @patch("database.psycopg2.connect")
    def test_passes_dsn_verbatim(self, mock_connect: MagicMock, db: DatabaseProbe) -> None:
    ...
    @patch("database.psycopg2.connect")
    def test_preserves_query_string_params(self, mock_connect: MagicMock) -> None:
    ...
    @patch("database.psycopg2.connect")
    def test_noop_when_already_connected(self, mock_connect: MagicMock, db: DatabaseProbe) -> None:
        db.conn = MagicMock()
    ...
    @patch("database.psycopg2.connect")
    def test_retries_on_operational_error(
        self, mock_connect: MagicMock, mock_sleep: MagicMock, db: DatabaseProbe
    ) -> None:
        mock_conn = MagicMock()
    ...
    @patch("database.time.sleep")
    @patch("database.psycopg2.connect")
    def test_raises_after_max_retries(self, mock_connect: MagicMock, mock_sleep: MagicMock, db: DatabaseProbe) -> None:
...
class TestDisconnect:
    ...
    def test_closes_connection(self, connected_db: DatabaseProbe) -> None:
        conn = cast(MagicMock, connected_db.conn)
    ...
    def test_noop_when_not_connected(self, db: DatabaseProbe) -> None:
...
class TestEnsureSchema:
    ...
    def _mock_cursor(self, db: DatabaseProbe, fetchone_value: tuple[str | None]):
        mock_cur = MagicMock()
        mock_cur.fetchone.return_value = fetchone_value
        cast(MagicMock, db.conn).cursor.return_value.__enter__ = MagicMock(return_value=mock_cur)
        cast(MagicMock, db.conn).cursor.return_value.__exit__ = MagicMock(return_value=False)
        return mock_cur

    def test_noop_when_tables_exist(self, connected_db: DatabaseProbe) -> None:
        ...
        cast(MagicMock, connected_db.conn).commit.assert_not_called()

    def test_applies_initial_sql_when_missing(self, connected_db: DatabaseProbe) -> None:
        ...
        cast(MagicMock, connected_db.conn).commit.assert_called_once()

    def test_rollback_on_error(self, connected_db: DatabaseProbe) -> None:
        mock_cur = self._mock_cursor(connected_db, (None,))
        ...
        cast(MagicMock, connected_db.conn).rollback.assert_called_once()
        cast(MagicMock, connected_db.conn).commit.assert_not_called()
...
class TestGetProcessedFiles:
    ...
    def test_returns_set_of_filenames(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()
        mock_cur.fetchall.return_value = [("file1.zip",), ("file2.zip",)]
        cast(MagicMock, connected_db.conn).cursor.return_value.__enter__ = MagicMock(return_value=mock_cur)
        cast(MagicMock, connected_db.conn).cursor.return_value.__exit__ = MagicMock(return_value=False)

        result = connected_db.get_processed_files("2024-01")

        assert result == {"file1.zip", "file2.zip"}

    def test_raises_on_error(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()
...
class TestMarkProcessed:
    ...
    def test_inserts_and_commits(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()
        cast(MagicMock, connected_db.conn).cursor.return_value.__enter__ = MagicMock(return_value=mock_cur)
        cast(MagicMock, connected_db.conn).cursor.return_value.__exit__ = MagicMock(return_value=False)
...
class TestClearProcessedFiles:
    ...
    def test_deletes_and_commits(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()
        cast(MagicMock, connected_db.conn).cursor.return_value.__enter__ = MagicMock(return_value=mock_cur)
        cast(MagicMock, connected_db.conn).cursor.return_value.__exit__ = MagicMock(return_value=False)
...
class TestTruncateTable:
    ...
    def test_truncates_and_tracks(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()
        cast(MagicMock, connected_db.conn).cursor.return_value.__enter__ = MagicMock(return_value=mock_cur)
        cast(MagicMock, connected_db.conn).cursor.return_value.__exit__ = MagicMock(return_value=False)
        ...
        cast(MagicMock, connected_db.conn).commit.assert_called_once()
        assert "empresas" in connected_db.truncated_tables

    def test_pre_truncated_constructor_param(self) -> None:
        """Database should accept pre_truncated tables via constructor."""
        db = DatabaseProbe("postgresql://test", pre_truncated={"empresas", "socios"})
        assert db.truncated_tables == {"empresas", "socios"}


class TestBulkUpsert:
    """Test bulk upsert with temp table strategy."""

    def test_skips_empty_dataframe(self, connected_db: DatabaseProbe) -> None:
        df = pl.DataFrame({"col": []})

        connected_db.bulk_upsert(df, "test_table", ["col"])

        cast(MagicMock, connected_db.conn).cursor.assert_not_called()

    def test_happy_path_sequence(self, connected_db: DatabaseProbe) -> None:
...
class TestBulkInsert:
    ...
    def test_truncates_on_first_batch(self, connected_db: DatabaseProbe) -> None:
        df = pl.DataFrame({"codigo": ["001"]})
        mock_cur = MagicMock()
        cast(MagicMock, connected_db.conn).cursor.return_value.__enter__ = MagicMock(return_value=mock_cur)
        cast(MagicMock, connected_db.conn).cursor.return_value.__exit__ = MagicMock(return_value=False)
    ...
    def test_does_not_truncate_target_on_second_batch(self, connected_db: DatabaseProbe) -> None:
        ...
        connected_db.bulk_insert(df, "cnaes", ["codigo"])
        mock_cur.reset_mock()
        cast(MagicMock, connected_db.conn).reset_mock()
...
class TestGetPrimaryKeys:
    ...
    def test_queries_and_caches(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()
        mock_cur.fetchall.return_value = [("cnpj_basico",), ("cnpj_ordem",)]

        result1 = connected_db.get_primary_keys(mock_cur, "estabelecimentos")
        result2 = connected_db.get_primary_keys(mock_cur, "estabelecimentos")
...
class TestCopyToTemp:
    ...
    def test_strips_null_bytes(self, connected_db: DatabaseProbe) -> None:
...
class TestUpsertFromTemp:
    ...
    def test_generates_update_clause(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()

        connected_db.upsert_from_temp(mock_cur, "temp_tbl", "cnaes", ["codigo", "descricao"], ["codigo"])
    ...
    def test_do_nothing_when_all_columns_are_pks(self, connected_db: DatabaseProbe) -> None:
        mock_cur = MagicMock()

        connected_db.upsert_from_temp(mock_cur, "temp_tbl", "test", ["id"], ["id"])
...
def test_connect_rejects_nonpositive_retry_budget(retry_attempts: int) -> None:
    db = Database("postgresql://unused", retry_attempts=retry_attempts)
...
def test_schema_query_without_row_fails(connected_db: DatabaseProbe) -> None:
    conn = cast(MagicMock, connected_db.conn)
    conn.cursor.return_value.__enter__.return_value.fetchone.return_value = None
    with pytest.raises(RuntimeError, match="Schema query returned no row"):
        connected_db.ensure_schema()
